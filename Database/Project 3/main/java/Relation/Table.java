...
implements Serializable
{
	...
	public Table (String _name, String [] _attribute, Class [] _domain, String [] _key)
	{
		...
		// UNCOMMENT THE DESIRED STRUCTURE
		index = new TreeMap <> ();       						
	...
	public Table minus (Table table2){
		...
		for (Comparable [] tup: tuples){					// go through tuples in table
			boolean temp = true;
			for(Comparable [] tup2 : table2.tuples){		// go through tuples in table2
				if(tup.equals(tup2)) temp = false;			// if no match, set temp to false
			}
			if(temp) rows.add(tup);							// if match, add to rows
		}
	...
	public Table join (String attributes1, String attributes2, Table table2)
	{
		...
		if(t_attrs.length != u_attrs.length){ 									// if different lengths, print error message
		...
		for (Comparable [] tup: tuples){										// iterate through tuples
			Comparable [] temp = new Comparable[this.attribute.length + table2.attribute.length]; //create new temp array, combined size
	...
	public Table join (Table table2){
		...
		//      		d_length--; // domain reduction due to shared abilities to match columns
		...
		tuples.stream().forEach((tup) -> { 												// streams each tuple

			table2.tuples.stream().filter((tup2) -> (tup == tup2)).map((tup2) -> {		

				int [] attr = match(attribute);
				int [] attr2 = match(table2.attribute);

				Comparable[] nTuple = new Comparable [attribute.length + table2.attribute.length];
				...
				for (int i = 0; i < attr2.length; i++){
					nTuple [count] = tup2 [attr2 [i]];
					count++;
				}

				return nTuple;

			}).forEach((nTuple) -> {rows.add(nTuple);});
